...
class EmailService {
  constructor() {
    try {
      ...
      this.initializeTransporter(cleanPassword);
  ...
  async initializeTransporter(cleanPassword) {
    const smtpConfigs = [
      {
        name: 'Gmail Service',
        config: {
    ...
    this.transporter = nodemailer.createTransport(smtpConfigs[0].config);
    this.configUsed = smtpConfigs[0].name;
  ...
  async verifyConnection() {
    ...
    for (const smtpConfig of this.smtpConfigs) {
      try {
        ...
        if (testConnection) {
          console.log(`SUCCESS: ${smtpConfig.name} connection successful`);
          this.configUsed = smtpConfig.name;
          return true;
        }
      } catch (error) {
        console.log(`FAILED: ${smtpConfig.name} - ${error.message}`);
        continue;
      }
    }
  ...
  async sendEnquiryConfirmation(email, firstName, enquiryDetails) {
    ...
    try {
      ...
      console.log('Enquiry confirmation failed but continuing');
      return null;
    }
  }
}
